...
create  or replace  trigger ARENA_noDelete_trig
  ...
  on ARENA
begin
  DBMS_OUTPUT.put_line('ARENA_noDelete_trig has fired');
...
create  or replace  trigger PLAYER_noDelete_trig
  ...
  on PLAYER
begin
  DBMS_OUTPUT.put_line('PLAYER_noDelete_trig has fired');
...
create  or replace  trigger TEAM_noDelete_trig
  ...
  on TEAM
begin
  DBMS_OUTPUT.put_line('TEAM_noDelete_trig has fired');
...
create  or replace  trigger ARENA_BeforInsert_trig
  ...
  FOR EACH ROW
begin
  DBMS_OUTPUT.put_line('ARENA_BeforInsert_trig has fired');
...
DECLARE
	teamCount int;
begin
	DBMS_OUTPUT.put_line('TEAM_noDuplicateName_trig has fired');
...
DECLARE
	arenaCount int;
begin
	DBMS_OUTPUT.put_line('ARENA_noDuplicateNameCity_trig has fired');
...
DECLARE
	errorCount int;
begin
	DBMS_OUTPUT.put_line('PLAYER_noNulls_trig has fired');
	errorCount := 0;
	If (:new.PNAME = '' OR :new.PNAME IS NULL) Then
		errorCount := errorCount + 1;
		DBMS_OUTPUT.put_line('Error: Player Name is Null');
	END IF;
	If (:new.EDUCATION = '' OR :new.EDUCATION IS NULL) Then
		errorCount := errorCount + 1;
		DBMS_OUTPUT.put_line('Error: Education Name is Null');
	END IF;
	If (:new.DRAFT_YEAR = 0 OR :new.DRAFT_YEAR IS NULL) Then
		errorCount := errorCount + 1;
		DBMS_OUTPUT.put_line('Error: Draft Year is not valid');
	END IF;
	If (:new.retire_year IS NOT NULL) Then
		errorCount := errorCount + 1;
		DBMS_OUTPUT.put_line('Error: Do not set retire year');
	END IF;
